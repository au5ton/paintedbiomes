...
public class Configs
{
  ...
  /**
   * Initialize the configuration directory and file locations. Call once for
   * example from preInit.
   */
  ...
  private static void setWorldDir()
  {
    ...
    // Unmapped function "func_240776_a_" is a returns the corresponding "Path" within the currently loaded "world" save folder
    // Used similarly here: https://github.com/FTBTeam/FTB-Backups/blob/6ccafcc1ed8286f52ab53071deb6d17295d07bbc/src/main/java/com/feed_the_beast/mods/ftbbackups/Backups.java#L208

    if(server != null && server.func_240776_a_(FolderName.DOT) != null)
    {
      File worldDir = server.func_240776_a_(FolderName.DOT).toFile();
  ...
  /**
   * Re-create the configuration instances and re-read all the configuration
   * files. Can only be called after calling setConfigDir() once.
   */
  ...
  private static void loadPerDimensionConfigs() {
    ...
    for (int dimension : mainConfig.enabledInDimensions) {
      ...
      if (conf == null) {
        ...
        if (file.exists() && file.isFile()) {
          PER_DIM_CONFIGS_GLOBAL.put(dimension,
              new Configs(globalConfigDir, dimension).copyFrom(mainConfig).loadConfigs());
        }
      }
    }
    ...
    for (int dimension : mainConfig.enabledInDimensions) {
      ...
      if (conf == null) {
        ...
        if (file.exists() && file.isFile()) {
          PER_DIM_CONFIGS_WORLD.put(dimension,
              new Configs(worldConfigDir, dimension).copyFrom(mainConfig).loadConfigs());
        }
      }
    }
  }
  ...
  private void readColorToBiomeMappings(Configuration conf) {
    ...
    // Iterate over the biome registry and add a color-to-biome mapping for all
    // biomes
    for (Map.Entry<ResourceLocation, Biome> entry : ForgeRegistries.BIOMES.getEntries()) {
      ...
      // No mapping found in the config, add a default mapping, so that all the
      // existing biomes will get added to the config
      else {
      ...
      if (oldId != -1) {
        ...
        PaintedBiomes.logger
            .warn(String.format("The color %06X (%d), attempted to use for biome '%s' (ID: %d), is already in use!",
        ...
        // For simplicity, when generating terrain, the biome is always read from the
        // mapping, even in case of a red channel mapping.
        // So basically we want to always add all the existing biomes to the
        // color-to-biome map.
  ...
  private int checkAndFixConfigValueInt(String configName, Property prop, int min, int max, int defaultValue) {
    ...
    if (value < min || value > max) {
      PaintedBiomes.logger.warn("Invalid config value for '{}': '{}', setting it to '{}'", configName, value,
          defaultValue);
      value = defaultValue;
      prop.set(value);
    }
